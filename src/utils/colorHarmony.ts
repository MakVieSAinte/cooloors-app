...
export type HarmonyType = 'analogous' | 'monochromatic' | 'complementary' | 'triad' | 'tetrad' | 'random'
...
export function generatePalette(
  ...
  switch (harmony) {
    case 'analogous':
      ...
      break
    case 'monochromatic':
      colors = chroma.scale([chroma(baseColor).brighten(2), baseColor, chroma(baseColor).darken(2)]).mode('lab').colors(count)
      break
    case 'complementary':
    ...
    case 'tetrad':
      ...
      colors = [baseColor, tetrad, chroma(baseColor).set('hsl.h', '+180').hex(), chroma(baseColor).set('hsl.h', '+270').hex()]
      colors = chroma.scale(colors).mode('lab').colors(count)
      break
    case 'random':
    default:
